 ...
 *
 * @async
 * @function fetchInsertOrUpdateSuperficiaisSync
 ...
 * `/services/insert-or-update-superficial-sync`. O método utilizado é `GET` (pode ser alterado para `POST`, caso necessário).
 ...
 * fetchInsertOrUpdateSuperficiaisSync(users)
 *   .then((message) => console.log(message))
 *   .catch((error) => console.error(error));
 */

const fetchInsertOrUpdateSuperficiaisSync = async () => {

     // Make the API request to the backend
     const response = await fetch('http://localhost:3000/services/insert-or-update-superficial-sync', {
...
export default fetchInsertOrUpdateSuperficiaisSync;