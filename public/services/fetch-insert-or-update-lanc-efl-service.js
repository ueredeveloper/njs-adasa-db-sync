 ...
 *
 * @async
 * @function fetchInsertOrUpdateLancEflService
 ...
 * `/services/insert-or-update-superficial`. O método utilizado é `GET` (pode ser alterado para `POST`, caso necessário).
 ...
 * fetchInsertOrUpdateLancEflService(users)
 *   .then((message) => console.log(message))
 *   .catch((error) => console.error(error));
 */

const fetchInsertOrUpdateLancEflService = async () => {

     // Make the API request to the backend
     const response = await fetch('http://localhost:3000/services/insert-or-update-lancamento-efluentes', {
...
export default fetchInsertOrUpdateLancEflService;